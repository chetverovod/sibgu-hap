...
DIR_READMES = {
    ...
    "scenario": dedent(
        ...
        - `topology`: dynamic network topology (`.dnt`) files.
    ...
    "scenario/topology": dedent(
        """\
        # scenario/topology

        Dynamic network topology files for the scenario.

        ## Typical Content
        - A `.dnt` archive generated by `dntgenerator.py`.
        - Optional alternative topology files.
...
def _dntgenerator_path() -> Path:
    return Path(__file__).resolve().parent / "dntgenerator.py"


def _validate_dnt_with_generator(dnt_path: Path) -> None:
    generator = _dntgenerator_path()
    if not generator.is_file():
        raise FileNotFoundError(f"dntgenerator.py not found: {generator}")
    result = subprocess.run(
        [sys.executable, str(generator), "--validate", str(dnt_path)],
        check=False,
        text=True,
        capture_output=True,
    )
    if result.returncode != 0:
        ...
        raise ValueError(
            f"Topology .dnt validation failed for: {dnt_path}\n{details}".strip()
        )


def _generate_dnt_template(output_dnt: Path, force: bool = True) -> None:
    generator = _dntgenerator_path()
    if not generator.is_file():
        raise FileNotFoundError(f"dntgenerator.py not found: {generator}")
    cmd = [sys.executable, str(generator), "--template", "--output", str(output_dnt)]
    if force:
        cmd.append("--force")
    ...
    if result.returncode != 0:
        ...
        if result.stderr and result.stderr.strip():
            details = (details + "\n" + result.stderr.strip()).strip()
        raise RuntimeError(
            f"Failed to generate topology template via dntgenerator.py\n{details}".strip()
        )


def build_task_tree(
    task_root: Path,
    topology_dnt: Path | None = None,
    include_topology_template: bool = False,
) -> None:
    ...
    for rel_dir in ALL_DIRS:
        if rel_dir == "scenario/topology":
            # Keep topology directory truly empty unless a .dnt is explicitly provided/generated.
            continue
    ...
    topology_dir = task_root / "scenario/topology"
    if include_topology_template:
        _generate_dnt_template(topology_dir / "topology_template.dnt", force=True)
    elif topology_dnt is not None:
        src = topology_dnt.resolve()
        if not src.exists():
            raise FileNotFoundError(f"Topology .dnt file not found: {src}")
        _validate_dnt_with_generator(src)
        shutil.copy2(src, topology_dir / src.name)


def create_tsk_archive(output_path: Path, force: bool, topology_dnt: Path | None = None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="sibgu_task_") as tmp_dir:
        ...
        build_task_tree(tmp_root, topology_dnt=topology_dnt, include_topology_template=False)
...
def create_task_template_dir(output_dir: Path, force: bool) -> Path:
    ...
    build_task_tree(output_dir, topology_dnt=None, include_topology_template=True)
    return output_dir
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        epilog=dedent(
            ...
            Examples:
              ...
              createtask.py create task1.tsk --topology-dnt ./net/topology.dnt --force
    ...
    create_parser = subparsers.add_parser(
        ...
        epilog=dedent(
            """\
            Example:
              createtask.py create my_task.tsk --topology-dnt ./topology.dnt --force
            """
        ),
    )
    ...
    create_parser.add_argument(
        ...
        default=None,
        help=(
            "Path to .dnt dynamic topology file to place into scenario/topology. "
            "If omitted, scenario/topology stays empty."
        ),
    )
...
def main() -> int:
    ...
    try:
        if args.command == "create":
            create_tsk_archive(args.output, force=args.force, topology_dnt=args.topology_dnt)
